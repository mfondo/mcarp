...
void leaderChanged(int isLeader) {
    ...
    char *leaderCommandWithStatus = (char *)malloc((strlen(leaderCommand) + 2) * sizeof(char));
    strcpy(leaderCommandWithStatus, leaderCommand);
    strcat(leaderCommandWithStatus, isLeader ? " 1" : " 0");
	...
	while(1) {
		if(isLeader) {
			...
			while(1) {			
				...
				pollResult = poll(&fd, 1, pollTimeout);
				if(pollResult < 0) {
					...
					if(fd.revents & POLLIN) {
						if(recvHeartbeat()) {
							...
							isLeader = 0;//TODO call user script for !isLeader
